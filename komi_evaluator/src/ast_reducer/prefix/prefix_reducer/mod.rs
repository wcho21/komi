...
pub fn reduce_num<F>(operand: &Ast, prefix_location: &Range, env: &Environment, get_kind: F) -> ResVal
where
    F: Fn(f64) -> ValueKind,
{
...
pub fn reduce_bool<F>(operand: &Ast, prefix_location: &Range, env: &Environment, get_kind: F) -> ResVal
where
    F: Fn(bool) -> ValueKind,
{
...
fn reduce<T, F, G>(operand: &Ast, prefix_location: &Range, env: &Environment, reduce_operand: F, get_kind: G) -> ResVal
where
