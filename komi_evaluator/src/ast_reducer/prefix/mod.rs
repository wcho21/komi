...
pub fn reduce_plus(operand: &Ast, prefix_location: &Range, env: &Environment) -> ResVal {
    prefix_reducer::reduce_num(operand, prefix_location, env, |v| ValueKind::Number(v))
}

/// Reduces the operand `operand` of a plus prefix to a value, with its location spanning from the prefix to the operand.
pub fn reduce_minus(operand: &Ast, prefix_location: &Range, env: &Environment) -> ResVal {
    prefix_reducer::reduce_num(operand, prefix_location, env, |v| ValueKind::Number(-v))
}

/// Reduces the operand `operand` of a plus prefix to a value, with its location spanning from the prefix to the operand.
pub fn reduce_bang(operand: &Ast, prefix_location: &Range, env: &Environment) -> ResVal {
