...
pub fn reduce_ast(ast: &Ast, env: &Environment) -> ResVal {
...
mod tests {
    ...
    #[case::num(
        ...
        mkast!(prog loc 0, 0, 0, 2, vec![
            mkast!(identifier "사과".to_string(), loc 0, 0, 0, 2),
        ]),
    ...
    fn single_identifier(#[case] ast: Box<Ast>, #[case] env: Environment, #[case] expected: Value) {
        assert_eval!(&ast, &env, expected);
    }
