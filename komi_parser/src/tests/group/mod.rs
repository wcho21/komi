...
/// Failure cases: single parenthesis.
#[rstest]
#[case::lparen(
    // Represents `(`.
    vec![
...
fn fail_single_paren(#[case] tokens: Vec<Token>, #[case] error: ParseError) {
    assert_parse_fail!(&tokens, error);
}

/// Failure cases: unmatched parentheses.
#[rstest]
...
fn fail_unmatched(#[case] tokens: Vec<Token>, #[case] error: ParseError) {
    assert_parse_fail!(&tokens, error);
}
